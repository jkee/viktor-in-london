 ...
 *   newbuilds — reviewed new-build/conversion candidates (🏢 badges)
...
function popupHtml(title, lines, footnote, titleHref) {
  ...
  return (
    ...
    lines.filter(Boolean).map(l => {
      const { text, cls } = typeof l === 'string' ? { text: l, cls: '' } : l;
      return `<div${cls ? ` class="${cls}"` : ''}>${escapeHtml(text)}</div>`;
    }).join('') +
...
/* ============================== New-builds ============================== */
/* Reviewed records from the london-newbuilds research project
 * (data/newbuilds.js is generated by scripts/build-newbuilds.py). Three tiers:
 *   B — investigate: navy 🏢, with "Verify: …" open due-diligence gates;
 *   C probably-ok holds: mostly future schemes (dashed 🏗) plus amber ⚠ for
 *     unresolved safety status;
 *   C probably-bad holds: grey demoted badge with the specific issue as a
 *     red warning — famous buildings people will look for, flagged not hidden.
 * Tags name the thing to check rather than pass a verdict, and are dated. */

function newbuildBadgeAndTag(d) {
  if (d.lane === 'B') {
    return {
      emoji: '🏢', variant: 'newbuilds', status: 'B — investigate',
      tag: d.verify && d.verify.length
        ? { text: 'Verify: ' + d.verify.map(g => g.replace(/_/g, ' ')).join(', '), cls: 'tag' }
        : null
    };
  }
  const c = { status: 'C — hold' };
  switch (d.bucket) {
    case 'not-yet-delivered':
      return { ...c, emoji: '🏗', variant: 'newbuilds-future',
               tag: { text: 'Future — not yet delivered', cls: 'tag' } };
    case 'safety-unresolved':
      return { ...c, emoji: '🏢', variant: 'newbuilds-warn',
               tag: { text: '⚠ Safety status unresolved — check current FRAEW/EWS1', cls: 'warn' } };
    case 'safety-defect-confirmed':
      return { ...c, emoji: '🏢', variant: 'newbuilds-bad',
               tag: { text: '⚠ Fire-safety defect on record — verify remediation completed', cls: 'warn' } };
    case 'identity-evidence-gap':
      return { ...c, emoji: '🏢', variant: 'newbuilds-muted',
               tag: { text: 'Record incomplete', cls: 'tag' } };
    default:  // operational-failure and any future probably-bad bucket
      return { ...c, emoji: '🏢', variant: 'newbuilds-bad',
               tag: { text: '⚠ Residents report problems — see note', cls: 'warn' } };
  }
}

const newbuilds = L.layerGroup((window.NEWBUILDS || []).map(d => {
  const { emoji, variant, status, tag } = newbuildBadgeAndTag(d);
  const marker = L.marker([d.lat, d.lng], { icon: badgeIcon(emoji, variant, false), title: d.name });
  marker.bindPopup(popupHtml(
    d.name,
    [
      tag,
      ...
      d.note ? { text: d.note, cls: 'issue' } : null
    ],
    `${status} · ${d.id}` + (d.asof ? ` · as of ${d.asof}` : ''),
    d.url
  ), { maxHeight: 280 });
  return marker;
}));  // off by default — toggled from the panel
