 ...
 *   newbuilds — reviewed new-build/conversion candidates (navy dots, canvas)
...
/* ============================== New-builds ============================== */
/* Reviewed "B — investigate" candidates from the london-newbuilds research
 * project (data/newbuilds.js is generated by scripts/build-newbuilds.py from
 * the frozen zone-1-2-v1 release). ~530 markers, so they share one canvas
 * renderer instead of getting a DOM badge each. */

const newbuildsRenderer = L.canvas({ padding: 0.4 });
const newbuilds = L.layerGroup((window.NEWBUILDS || []).map(d => {
  const marker = L.circleMarker([d.lat, d.lng], {
    renderer: newbuildsRenderer,
    radius: 5,
    color: '#fff',
    weight: 1.5,
    fillColor: '#10069f',
